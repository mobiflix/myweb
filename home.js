...
// ===== YEAR PAGINATION SETTINGS =====
const START_YEAR = 2026;
const END_YEAR = 2000;
const MIN_VOTES = 50; // Para legit ratings lang
...
// ============================================================
// TOP 10 (CURRENT YEAR, HIGHEST RATED)
// ============================================================
...
async function fetchTop10Movies() {
  ...
  const res = await fetch(url);
  const data = await res.json();
  return { results: filterNonIndian(data.results).slice(0, 10) };
}

// Top 10 TV Shows 2026
async function fetchTop10TV() {
  const currentYear = new Date().getFullYear();
  const today = new Date().toISOString().split('T')[0];
...
// Top 10 KDrama 2026
async function fetchTop10Kdrama() {
  const currentYear = new Date().getFullYear();
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
  ...
  const res = await fetch(url);
  const data = await res.json();
  return { results: (data.results || []).map(function(x) {
    x.media_type = 'tv';
    return x;
  }).slice(0, 10) };
}
...
// Top 10 Anime 2026
async function fetchTop10Anime() {
  const currentYear = new Date().getFullYear();
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
    `&with_genres=16` +
...
// ============================================================
// PER-YEAR FETCHERS (For View All pages)
// ============================================================

// Movies by year (highest rated)
async function fetchMoviesByYear(year, page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/movie?api_key=${API_KEY}` +
...
// TV Shows by year (highest rated)
async function fetchSeriesByYear(year, page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
  ...
  const res = await fetch(url);
  const data = await res.json();
  const results = (data.results || []).map(function(x) {
    x.media_type = 'tv';
    return x;
  });
...
// KDrama by year (highest rated)
async function fetchKdramaByYear(year, page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
    `&with_origin_country=KR` +
    `&first_air_date_year=${year}` +
...
// Anime by year (highest rated, both TV + Movie)
async function fetchAnimeByYear(year, mediaType, page) {
  const today = new Date().toISOString().split('T')[0];
  ...
  if (mediaType === 'movie') {
    url += `&primary_release_year=${year}&primary_release_date.lte=${today}`;
  } else {
    url += `&first_air_date_year=${year}&first_air_date.lte=${today}`;
  }
...
// ============================================================
// FETCHERS WITH FILTERS (Override per-year kung may year filter)
// ============================================================

async function fetchMoviesWithFilters(filters, page) {
  const today = new Date().toISOString().split('T')[0];
  const sortBy = filters.sort || 'vote_average.desc';
  ...
  if (sortBy.indexOf('vote_average') === 0) {
    url += `&vote_count.gte=${MIN_VOTES}`;
  }
...
async function fetchSeriesWithFilters(filters, page) {
  const today = new Date().toISOString().split('T')[0];
  const sortBy = filters.sort || 'vote_average.desc';
...
async function fetchKdramaWithFilters(filters, page) {
  const today = new Date().toISOString().split('T')[0];
  const sortBy = filters.sort || 'vote_average.desc';
...
async function fetchAnimeWithFilters(filters, page) {
  const today = new Date().toISOString().split('T')[0];
  const sortBy = filters.sort || 'vote_average.desc';
...
function renderTop10(items, containerId, defaultMediaType) {
  const container = document.getElementById(containerId);
  if (!container) return;
  container.innerHTML = '';
  ...
  items.slice(0, 10).forEach(function(item, index) {
    ...
    if (!item.media_type && defaultMediaType) {
      item.media_type = defaultMediaType;
    }
...
const FILTER_PANEL_MAP = {
  ...
  'kdrama': 'kdrama-filter-panel',
  'anime': 'anime-filter-panel'
};
...
function getFilterValues(pageKey) {
  ...
  return {
    ...
    sort: sortEl ? sortEl.value : 'vote_average.desc'
  };
}
...
function applyFilters(pageKey) {
  ...
  if (pageKey === 'view-all') {
    ...
    moviesYearState.filters = filters;
    moviesYearState.year = filters.year ? parseInt(filters.year, 10) : START_YEAR;
    moviesYearState.page = 1;
    moviesYearState.hasMore = true;
    moviesYearState.seenIds = new Set();
    moviesYearState.yearExhausted = false;
    moviesYearState.totalAdded = 0;
    ...
    seriesYearState.filters = filters;
    seriesYearState.year = filters.year ? parseInt(filters.year, 10) : START_YEAR;
    seriesYearState.page = 1;
    seriesYearState.hasMore = true;
    seriesYearState.seenIds = new Set();
    seriesYearState.yearExhausted = false;
    seriesYearState.totalAdded = 0;
    ...
    loadVivamaxBatch();
  } else if (pageKey === 'kdrama') {
    kdramaPageState.filters = filters;
    kdramaPageState.year = filters.year ? parseInt(filters.year, 10) : START_YEAR;
    kdramaPageState.page = 1;
    kdramaPageState.hasMore = true;
    kdramaPageState.seenIds = new Set();
    kdramaPageState.yearExhausted = false;
    kdramaPageState.totalAdded = 0;
    ...
    animePageState.filters = filters;
    animePageState.year = filters.year ? parseInt(filters.year, 10) : START_YEAR;
    animePageState.page = 1;
    animePageState.hasMore = true;
    animePageState.seenIds = new Set();
    animePageState.yearExhausted = false;
    animePageState.totalAdded = 0;
...
function clearFilters(pageKey) {
  ...
  if (sortEl) sortEl.value = 'vote_average.desc';
  ...
  if (pageKey === 'view-all') {
    ...
    moviesYearState.filters = {};
    moviesYearState.year = START_YEAR;
    moviesYearState.page = 1;
    moviesYearState.hasMore = true;
    moviesYearState.seenIds = new Set();
    moviesYearState.yearExhausted = false;
    ...
    seriesYearState.filters = {};
    seriesYearState.year = START_YEAR;
    seriesYearState.page = 1;
    seriesYearState.hasMore = true;
    seriesYearState.seenIds = new Set();
    seriesYearState.yearExhausted = false;
    ...
    kdramaPageState.filters = {};
    kdramaPageState.year = START_YEAR;
    kdramaPageState.page = 1;
    kdramaPageState.hasMore = true;
    kdramaPageState.seenIds = new Set();
    kdramaPageState.yearExhausted = false;
    ...
    animePageState.filters = {};
    animePageState.year = START_YEAR;
    animePageState.page = 1;
    animePageState.hasMore = true;
    animePageState.seenIds = new Set();
    animePageState.yearExhausted = false;
...
// MOVIES - per year
async function loadMoviesYearBatch() {
  if (moviesYearState.loading || !moviesYearState.hasMore) return;
  moviesYearState.loading = true;
  document.getElementById('movies-page-loading').style.display = 'block';

  try {
    const filters = moviesYearState.filters || {};
    let data;

    if (filters.year || filters.rating || filters.genre || filters.country) {
      // May filter — gamitin ang fetchMoviesWithFilters
      data = await fetchMoviesWithFilters(filters, moviesYearState.page);
    } else {
      // Walang filter — per-year
      if (moviesYearState.year < END_YEAR) {
        moviesYearState.hasMore = false;
        document.getElementById('movies-page-end').style.display = 'block';
        return;
      }

      data = await fetchMoviesByYear(moviesYearState.year, moviesYearState.page);

      if (!data.results || data.results.length === 0) {
        // Walang laman yung year na 'to — baba sa next year
        moviesYearState.year -= 1;
        moviesYearState.page = 1;
        if (moviesYearState.year < END_YEAR) {
          moviesYearState.hasMore = false;
          document.getElementById('movies-page-end').style.display = 'block';
        } else {
          // Recursive — subukan yung next year
          moviesYearState.loading = false;
          document.getElementById('movies-page-loading').style.display = 'none';
          return loadMoviesYearBatch();
        }
        return;
      }
    }
    ...
    data.results.forEach(function(item) {
      if (!item.poster_path) return;
      if (moviesYearState.seenIds.has(item.id)) return;
      moviesYearState.seenIds.add(item.id);
      item.media_type = 'movie';
      moviesYearState.totalAdded += 1;
      const img = document.createElement('img');
      img.src = `${IMG_W500}${item.poster_path}`;
      img.alt = item.title || item.name;
      img.loading = 'lazy';
    ...
    // Kung naubos na yung page ng current year (maxPages reached), baba sa next year
    if (!filters.year && moviesYearState.page > (data.total_pages || 1)) {
      moviesYearState.year -= 1;
      moviesYearState.page = 1;
      if (moviesYearState.year < END_YEAR) {
        moviesYearState.hasMore = false;
...
// SERIES - per year
async function loadSeriesYearBatch() {
  if (seriesYearState.loading || !seriesYearState.hasMore) return;
  seriesYearState.loading = true;
  document.getElementById('series-page-loading').style.display = 'block';

  try {
    const filters = seriesYearState.filters || {};
    let data;

    if (filters.year || filters.rating || filters.genre || filters.country) {
      data = await fetchSeriesWithFilters(filters, seriesYearState.page);
    } else {
      if (seriesYearState.year < END_YEAR) {
        seriesYearState.hasMore = false;
        document.getElementById('series-page-end').style.display = 'block';
        return;
      }

      data = await fetchSeriesByYear(seriesYearState.year, seriesYearState.page);

      if (!data.results || data.results.length === 0) {
        seriesYearState.year -= 1;
        seriesYearState.page = 1;
        if (seriesYearState.year < END_YEAR) {
          seriesYearState.hasMore = false;
          document.getElementById('series-page-end').style.display = 'block';
        } else {
          seriesYearState.loading = false;
          document.getElementById('series-page-loading').style.display = 'none';
          return loadSeriesYearBatch();
        }
        return;
      }
    }
    ...
    data.results.forEach(function(item) {
      if (!item.poster_path) return;
      if (seriesYearState.seenIds.has(item.id)) return;
      seriesYearState.seenIds.add(item.id);
      item.media_type = 'tv';
      seriesYearState.totalAdded += 1;
      const img = document.createElement('img');
      img.src = `${IMG_W500}${item.poster_path}`;
      img.alt = item.title || item.name;
      img.loading = 'lazy';
    ...
    if (!filters.year && seriesYearState.page > (data.total_pages || 1)) {
      seriesYearState.year -= 1;
      seriesYearState.page = 1;
      if (seriesYearState.year < END_YEAR) {
        seriesYearState.hasMore = false;
...
// KDRAMA - per year
async function loadKdramaYearBatch() {
  ...
  try {
    const filters = kdramaPageState.filters || {};
    let data;

    if (filters.year || filters.rating || filters.genre || filters.country) {
      data = await fetchKdramaWithFilters(filters, kdramaPageState.page);
    } else {
      if (kdramaPageState.year < END_YEAR) {
        kdramaPageState.hasMore = false;
        document.getElementById('kdrama-page-end').style.display = 'block';
        return;
      }

      data = await fetchKdramaByYear(kdramaPageState.year, kdramaPageState.page);

      if (!data.results || data.results.length === 0) {
        kdramaPageState.year -= 1;
        kdramaPageState.page = 1;
        if (kdramaPageState.year < END_YEAR) {
          kdramaPageState.hasMore = false;
          document.getElementById('kdrama-page-end').style.display = 'block';
    ...
    if (!filters.year && kdramaPageState.page > (data.total_pages || 1)) {
      kdramaPageState.year -= 1;
      kdramaPageState.page = 1;
      if (kdramaPageState.year < END_YEAR) {
        kdramaPageState.hasMore = false;
...
// ANIME - per year
async function loadAnimeYearBatch() {
  ...
  try {
    const filters = animePageState.filters || {};
    let data;

    if (filters.year || filters.rating || filters.genre || filters.country) {
      data = await fetchAnimeWithFilters(filters, animePageState.page);
    } else {
      if (animePageState.year < END_YEAR) {
        animePageState.hasMore = false;
        document.getElementById('anime-page-end').style.display = 'block';
        return;
      }

      // Alternate TV + Movie para sa bawat year
      const mediaType = (animePageState.page % 2 === 0) ? 'movie' : 'tv';
      data = await fetchAnimeByYear(animePageState.year, mediaType, animePageState.page);

      if (!data.results || data.results.length === 0) {
        animePageState.year -= 1;
        animePageState.page = 1;
        if (animePageState.year < END_YEAR) {
          animePageState.hasMore = false;
          document.getElementById('anime-page-end').style.display = 'block';
    ...
    if (!filters.year && animePageState.page > (data.total_pages || 1)) {
      animePageState.year -= 1;
      animePageState.page = 1;
      if (animePageState.year < END_YEAR) {
        animePageState.hasMore = false;
...
function kdramaPageScrollHandler() {
  ...
  const page = document.getElementById('kdrama-page');
  if (!page) return;
  if (page.scrollTop + page.clientHeight >= page.scrollHeight - 300) loadKdramaYearBatch();
}
...
function animePageScrollHandler() {
  ...
  const page = document.getElementById('anime-page');
  if (!page) return;
  if (page.scrollTop + page.clientHeight >= page.scrollHeight - 300) loadAnimeYearBatch();
}
...
function moviesPageScrollHandler() {
  if (!moviesYearState.initialized || moviesYearState.loading || !moviesYearState.hasMore) return;
  const page = document.getElementById('movies-page');
  if (!page) return;
  if (page.scrollTop + page.clientHeight >= page.scrollHeight - 300) loadMoviesYearBatch();
}
...
function seriesPageScrollHandler() {
  if (!seriesYearState.initialized || seriesYearState.loading || !seriesYearState.hasMore) return;
  const page = document.getElementById('series-page');
  if (!page) return;
  if (page.scrollTop + page.clientHeight >= page.scrollHeight - 300) loadSeriesYearBatch();
}
...
async function init() {
  try {
    ...
    const [moviesTop10, tvTop10, kdramaTop10, animeTop10, ongoingData, completedData, vivamaxData] = await Promise.all([
      fetchTop10Movies(),
      fetchTop10TV(),
      fetchTop10Kdrama(),
      fetchTop10Anime(),
    ...
    // Banner — random mula sa top 5 movies 2026
    if (moviesTop10.results.length > 0) {
      const randomIndex = Math.floor(Math.random() * Math.min(5, moviesTop10.results.length));
      displayBanner(moviesTop10.results[randomIndex]);
    }

    renderTop10(moviesTop10.results, 'top10-movies', 'movie');
    renderTop10(tvTop10.results, 'top10-tv', 'tv');
    renderTop10(kdramaTop10.results, 'top10-kdrama', 'tv');
    renderTop10(animeTop10.results, 'top10-anime', 'tv');
