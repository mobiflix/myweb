...
async function loadEpisodesSection(item) {
  ...
  // ✅ MAS RELIABLE NA CHECK: kung may 'name' at 'first_air_date' → TV
  ...
  // Siguraduhin na 'tv' ang media_type
  item.media_type = 'tv';
...
function renderTop10(items, containerId, mediaType) {
  ...
  released.slice(0, 10).forEach(function(item, index) {
    ...
    // ✅ SIGURADUHIN NA NAKA-SET ANG media_type
    if (mediaType) {
...
async function showDetails(item) {
  ...
  // ✅ MAS MAAAYOS NA MEDIA TYPE DETECTION
  if (!item.media_type) {
  ...
  // ✅ ITO ANG PINAKA-MAHALAGA PARA SA EPISODES
  if (mediaType === 'tv') {
...
async function init() {
  try {
    ...
    // ✅ I-SET ANG media_type BAGO I-RENDER
    ...
    animeData.results.forEach(function(item) { item.media_type = 'tv'; });
