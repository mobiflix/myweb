...
// ===== EXCLUDED TV GENRES (game shows, reality, variety, documentary, news, talk, soap, kids, animation) =====
const EXCLUDED_GENRES = [10764, 10767, 10763, 10766, 16, 99, 10762, 10768];

// ===== STREAMING PROVIDERS (Updated: tinanggal Paramount+ at Crunchyroll, idinagdag AMC+) =====
...
// ===== GENRE LIST (kapareho ng nasa bottom navigation) =====
const GENRE_LIST = [
...
// ============================================================
// FILTER EXCLUDED TV GENRES (game shows, reality, variety, etc.)
// ============================================================
...
async function fetchKDrama2026(page) {
  return fetchKoreanSeriesNewest(page);
}
...
async function fetchOngoingTV(page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&page=${page}&first_air_date.lte=${today}&vote_count.gte=50&with_status=0|1&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES.join(',')}`;
  const res = await fetch(url);
  const data = await res.json();
...
async function fetchCompletedTV(page) {
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&page=${page}&with_status=3|4&vote_count.gte=100&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES.join(',')}`;
  const res = await fetch(url);
  const data = await res.json();
  let results = filterReleased(data.results);
  results = filterTVSeries(results);
...
async function fetchByProvider(providerId, mediaType, page, sortBy) {
  const sort = sortBy || 'primary_release_date.desc';
  const today = new Date().toISOString().split('T')[0];
  let url = `${BASE_URL}/discover/${mediaType}?api_key=${API_KEY}&with_watch_providers=${providerId}&watch_region=US&page=${page}&sort_by=${sort}&without_original_language=${INDIAN_LANGS.join('|')}`;
...
async function generateNotifications() {
  ...
  try {
    ...
    const ongoingRes = await fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&first_air_date.lte=${todayStr}&vote_count.gte=50&with_status=0|1&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES.join(',')}&page=1`);
...
// ============================================================
// ADULT PAGE (Vivamax) - nasa Profile na ngayon
// ============================================================
