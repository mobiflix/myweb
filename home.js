...
// ============================================================
// HISTORY MANAGEMENT (para sa phone back button)
// ============================================================
...
// ============================================================
// EXIT CONFIRMATION (para sa phone back button sa homepage)
// ============================================================
...
window.addEventListener('popstate', function(e) {
  // Kung may bukas na layer (modal o page), isara lang ito
  ...
  // Double-check kung may natitirang bukas na UI
  const modal = document.getElementById('modal');
  const playerModal = document.getElementById('player-modal');
  const anyOpenPage = document.querySelector(
    '.view-all-page.open, .my-list-page.open, .more-page.open, ' +
    '.user-profile-page.open, .search-modal.open'
  );

  if ((modal && modal.style.display === 'flex') ||
      (playerModal && playerModal.classList.contains('open')) ||
      anyOpenPage) {
    closeAllPagesOnly();
    closeModalOnly();
    ...
    if (playerModalEl) playerModalEl.classList.remove('open');
    return;
  }
...
function openPlayerModal(url, title) {
  ...
  // I-reset muna ang iframe bago i-load ang bago
  iframe.src = 'about:blank';

  // Maliit na delay para tuluyang ma-clear ang previous video
  setTimeout(function() {
    iframe.src = url;
  }, 50);
  ...
  // I-push ang player layer (para sa phone back button)
  pushLayer('player');
}
...
function closePlayerModal() {
  ...
  if (iframe) {
    // Kailangan i-reset via about:blank para tuluyang mag-unload ang video
    iframe.src = 'about:blank';
  }
  ...
  // I-pop ang player layer kung ito ang nasa taas
...
function playNow() {
  ...
  console.log('[MobiFlix Player] Opening in modal:', embedURL);
...
function playEpisode(tvId, seasonNumber, episodeNumber) {
  ...
  console.log('[MobiFlix Episode] Opening in modal:', url);
...
function closeAllPagesOnly() {
  ...
  // Isara at i-reset ang player modal
  const playerModal = document.getElementById('player-modal');
