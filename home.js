...
// Para sa "Recently Added" badge — 30 araw mula ngayon
...
const STREAMING_PROVIDERS = [
  ...
  { name: 'Blumhouse', id: 129126, type: 'company', color: '#6d28d9' },
  { name: 'Marvel Studios', id: 420, type: 'company', color: '#e62429' }
];
...
const PROVIDER_LOGOS = {
  ...
  'Blumhouse': 'https://upload.wikimedia.org/wikipedia/commons/3/3a/Blumhouse_Productions_logo.svg',
...
// ============================================================
// ✅ BAGONG HELPERS PARA SA POSTER META
// ============================================================
...
// ✅ Gagawa ng poster card na may rating, year, at "Recently Added" badge
function createPosterCard(item, mediaType, extraClass) {
  ...
  card.className = 'poster-card' + (extraClass ? ' ' + extraClass : '');
  ...
  const recent = isRecentlyAdded(item);

  if (recent) {
    const badge = document.createElement('div');
    badge.className = 'poster-badge';
    badge.textContent = 'Recently Added';
  ...
  if (rating) {
    ...
    r.innerHTML = '<i class="fa fa-star"></i> ' + rating;
    meta.appendChild(r);
  }
...
// ✅ Para sa grid pages (view-all, my-list, etc.)
function createGridPoster(item, mediaType) {
  if (!item.poster_path) return null;
...
// ============================================================
// ✅ BAGONG HELPER: Tamang sort params depende sa media type
// ============================================================
function getSortParams(sortBy, mediaType) {
  ...
  if (sortBy === 'primary_release_date.desc') {
    if (mediaType === 'tv') {
      return '&sort_by=first_air_date.desc';
    }
  ...
  if (sortBy === 'primary_release_date.asc') {
    if (mediaType === 'tv') {
      return '&sort_by=first_air_date.asc';
    }
...
// ✅ BAGONG appendToList — gumagamit ng poster card
...
async function loadVivamaxBatch() {
  ...
  try {
    ...
    filterReleased(data.results).forEach(function(item) {
      ...
      const card = createGridPoster(item, 'movie');
...
async function loadGenrePageBatch() {
  ...
  try {
    ...
    filterReleased(data.results).forEach(function(item) {
      ...
      const card = createGridPoster(item, genre.media);
      if (card) grid.appendChild(card);
    });
...
async function loadOngoingPageBatch() {
  ...
  try {
    ...
    data.results.forEach(function(item) {
      ...
      item.media_type = 'tv';
      const card = createGridPoster(item, 'tv');
      if (card) grid.appendChild(card);
    });
...
async function loadProviderBatch() {
  ...
  try {
    ...
    // ✅ Kung company ang provider, with_companies ang gamitin
    let providerParam;
    if (providerPageState.providerType === 'company') {
      providerParam = `&with_companies=${providerPageState.providerId}`;
    } else {
      providerParam = `&with_watch_providers=${providerPageState.providerId}`;
    }

    let url = `${BASE_URL}/discover/${mediaType}?api_key=${API_KEY}` +
      providerParam +
      (providerPageState.providerType === 'company' ? '' : '&watch_region=US') +
      `&page=${apiPage}` +
      sortParams +
      `&without_original_language=${INDIAN_LANGS.join('|')}` +
      `&without_genres=${bannedList.join(',')}`;
    ...
    filterReleased(data.results).forEach(function(item) {
      ...
      item.media_type = mediaType;
      const card = createGridPoster(item, mediaType);
      if (card) grid.appendChild(card);
    });
...
function renderMyList() {
  ...
  list.forEach(function(item) {
    if (!item.poster_path) return;
    const card = createGridPoster(item);
    if (card) grid.appendChild(card);
  });
}
...
async function loadViewAllBatch() {
  ...
  try {
    ...
    if (hasFilters || hasCustomSort) {
      ...
      if (filters.genre) {
        filterParams += `&with_genres=${filters.genre}`;
      }
      if (filters.country) {
        filterParams += `&with_origin_country=${filters.country}`;
      }
    ...
    data.results.forEach(function(item) {
      ...
      const card = createGridPoster(item, config.media);
      if (card) grid.appendChild(card);
    });
