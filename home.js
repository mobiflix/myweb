...
window.addEventListener('popstate', function(e) {
  // PRIORITY 1: Kung bukas ang player modal, isara at ibalik sa details
  const playerModal = document.getElementById('player-modal');
  if (playerModal && playerModal.classList.contains('open')) {
    // Alisin ang player layer sa stack
    if (layerStack.length > 0 && layerStack[layerStack.length - 1].type === 'player') {
      layerStack.pop();
    }

    // Isara nang tuluyan ang player at i-reset ang iframe
    forceClosePlayer();

    // Ipakita muli ang details modal (kung bukas ito dati)
    const detailsModal = document.getElementById('modal');
    if (detailsModal && detailsModal.dataset.wasOpen === 'true') {
  ...
  // PRIORITY 2: Kung may ibang bukas na layer, isara
...
// ============================================================
// IN-APP PLAYER MODAL — KUMPLETONG FIX
// ============================================================

// TANDAAN: Kapag bubuksan ang player, i-remember kung bukas ang details modal
function openPlayerModal(url, title) {
  ...
  if (!modal || !iframe) {
    console.error('[MobiFlix Player] Player modal not found!');
    return;
  }

  // I-remember kung bukas ang details modal
  if (detailsModal && detailsModal.style.display === 'flex') {
    detailsModal.dataset.wasOpen = 'true';
  } else {
    detailsModal.dataset.wasOpen = 'false';
  }

  // Itago ang details modal habang nagpi-play
  if (detailsModal) detailsModal.style.display = 'none';

  // I-reset ang iframe bago i-load ang bago
  ...
  // I-push ang player layer
  layerStack.push({ type: 'player', data: null });
  history.pushState({ mobiflixLayer: layerStack.length, type: 'player' }, '');
}

// FORCE CLOSE — ginagamit ng popstate at ng back button
// Walang history involvement — puro DOM cleanup lang
function forceClosePlayer() {
  ...
  // Itago ang player modal
  if (modal) {
    modal.classList.remove('open');
    modal.style.display = '';  // i-reset sa CSS default (none)
  }

  // I-reset ang body overflow
  document.body.style.overflow = '';
  document.documentElement.style.overflow = '';
}

// Ito ang tatawagin ng BACK BUTTON sa player header
function handlePlayerBack() {
  // Direkta nang isara at ibalik sa details (walang history.back() para sigurado)
  forceClosePlayer();

  // Ibalik ang details modal kung bukas ito dati
  const detailsModal = document.getElementById('modal');
  if (detailsModal && detailsModal.dataset.wasOpen === 'true') {
    detailsModal.style.display = 'flex';
  ...
  // I-remove ang isang history entry (yung player entry)
  // Para hindi na kailangan mag-back ng phone
  try {
    history.replaceState({ mobiflixHome: true }, '', '#home');
  } catch (e) {}
}
...
function playNow() {
  ...
  const embedURL = isMovie
    ? ZXCSTREAM_MOVIE + currentItem.id
    : ZXCSTREAM_TV + currentItem.id;

  const title = currentItem.title || currentItem.name || 'Now Playing';

  openPlayerModal(embedURL, title);
}
...
function playEpisode(tvId, seasonNumber, episodeNumber) {
  ...
  const title = `S${seasonNumber} • E${episodeNumber}`;
...
async function showDetails(item) {
  ...
  document.getElementById('modal').dataset.wasOpen = 'true';
  document.body.style.overflow = 'hidden';
