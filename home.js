...
// ✅ BAGO: Most Watched — newest to oldest (sort_by=primary_release_date.desc)
async function fetchMostWatched(mediaType, page) {
  ...
  if (mediaType === 'movie') {
    url += `&primary_release_date.gte=${currentYear - 3}-01-01&primary_release_date.lte=${today}`;
  } else {
    url += `&first_air_date.gte=${currentYear - 3}-01-01&first_air_date.lte=${today}`;
  }
...
// ✅ BAGO: Ipapakita lang ang mga naipalabas na episodes
async function loadSeasonEpisodes(tvId, seasonNumber) {
  ...
  // ✅ Filter: ipakita lang ang mga may air_date na at naipalabas na
  const today = new Date().toISOString().split('T')[0];
...
// ✅ BAGO: Bubuksan ang player.html para sa episode
...
// ============================================================
// PLAY NOW — BUBUKAS SA PLAYER.HTML
// ============================================================
...
async function loadViewAllBatch() {
  ...
  try {
    ...
    if (hasFilters) {
      ...
      // ✅ BAGO: newest to oldest na ang default
      if (key === 'movies') {
    ...
    // ✅ BAGO: Hindi na nag-sort ng popularity — sundin ang API order (newest to oldest)
    data.results.forEach(function(item) {
      if (!item.poster_path) return;
