...
async function fetchKdrama(page, mode, filters) {
  filters = filters || {};
  ...
  if (mode === 'top10') {
    // Top 10 ng KASALUKUYANG TAON, sorted by popularity, today o earlier lang
    url += `&first_air_date_year=${currentYear}` +
      `&sort_by=popularity.desc` +
      `&vote_count.gte=10`;
  } else {
    // View All — default newest to oldest
...
async function fetchAnime(page, mode, filters) {
  filters = filters || {};
  const animeGenre = 16;
  const origin = 'JP';
  const currentYear = new Date().getFullYear();
  ...
  if (mode === 'top10') {
    // Top 10 ng KASALUKUYANG TAON, TV anime by popularity, today o earlier lang
    const url =
  ...
  // View All — alternate TV at Movie anime, default newest to oldest
  ...
  // ✅ I-exclude ang mga hindi pa ipinapalabas
  if (mediaType === 'movie') {
...
function openKdramaPage() {
  ...
  kdramaPageState = { page: 1, maxPages: 500, loading: false, hasMore: true, initialized: true, seenIds: new Set(), filters: {} };
...
async function loadKdramaPageBatch() {
  ...
  try {
    ...
    const data = await fetchKdrama(kdramaPageState.page, 'all', filters);
...
function openAnimePage() {
  ...
  animePageState = { page: 1, maxPages: 500, loading: false, hasMore: true, initialized: true, seenIds: new Set(), filters: {} };
...
async function loadAnimePageBatch() {
  ...
  try {
    ...
    const data = await fetchAnime(animePageState.page, 'all', filters);
...
async function showDetails(item) {
  ...
  // ✅ Mas maaasahang pag-detect ng media type
...
async function init() {
  try {
    ...
    renderTop10(animeData.results, 'top10-anime', 'tv');
