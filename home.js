...
// ===== EXCLUDED GENRES (Game show, Reality, Variety, Talk, Soap, Music, Documentary, News, Politics) =====
// TV: 10764 Reality, 10767 Talk, 10766 Soap, 10763 News, 10768 War & Politics, 10762 Kids (optional)
// Movie: 99 Documentary, 10402 Music, 10770 TV Movie
...
// Para sa Korean Series, idagdag din ang 16 (Animation) at 10762 (Kids)
...
function toggleFamilyMode() {
  ...
  // I-reload ang homepage para ma-apply agad
...
function populateCountryDropdowns() {
  ...
  // Populate Genre dropdowns (walang rating)
...
function filterMinRating(results, minRating) {
  const min = minRating || 5.0; // Default 5 stars minimum
  return (results || []).filter(function(item) {
...
function checkDailyUpdate() {
  ...
  if (lastUpdate !== today) {
    ...
    // Clear cached notifications para mag-refresh
...
async function fetchTrendingUS(type, page) {
  // Trending this week sa America (US)
  const res = await fetch(`${BASE_URL}/trending/${type}/week?api_key=${API_KEY}&page=${page}&region=US`);
  const data = await res.json();
  let results = applyAllFilters(data.results, type);
  return { results: results, total_pages: data.total_pages || 1 };
...
async function fetchMostWatched(mediaType, page) {
  ...
  if (mediaType === 'movie') {
    url += `&primary_release_date.lte=${today}` + `&without_genres=${EXCLUDED_GENRES_ALL.join(',')}`;
  } else {
    url += `&first_air_date.lte=${today}` + `&without_genres=${EXCLUDED_GENRES_TV.concat([99, 10402, 10770]).join(',')}`;
  }
...
async function fetchKoreanSeriesNewest(page) {
  ...
  results = filterMinRating(results, 5.0);
...
async function fetchByGenre(mediaType, genreId, page) {
  const url = `${BASE_URL}/discover/${mediaType}?api_key=${API_KEY}&with_genres=${genreId}&sort_by=popularity.desc&page=${page}&vote_average.gte=5&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_ALL.join(',')}`;
  const res = await fetch(url);
  const data = await res.json();
  let results = applyAllFilters(data.results, mediaType);
  return { results: results, total_pages: data.total_pages || 1 };
...
async function fetchByProvider(providerId, mediaType, page) {
  const url = `${BASE_URL}/discover/${mediaType}?api_key=${API_KEY}&with_watch_providers=${providerId}&watch_region=US&page=${page}&sort_by=popularity.desc&vote_average.gte=5&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_ALL.join(',')}`;
  const res = await fetch(url);
  const data = await res.json();
  let results = applyAllFilters(data.results, mediaType);
  return { results: results, total_pages: data.total_pages || 1 };
...
// ============================================================
// SEARCH (may Korean Series priority, Family Mode filter)
// ============================================================
...
async function searchTMDB() {
  ...
  searchTimeout = setTimeout(async () => {
    try {
      ...
      // Family Mode: filter adult content
      if (familyMode) {
      ...
      const finalResults = combined
        ...
        .filter(function(item) {
          return (item.vote_average || 0) >= 5;
        })
        .filter(function(item) {
          const genres = item.genre_ids || [];
          return !genres.some(function(g) { return EXCLUDED_GENRES_ALL.includes(g); });
        })
        .sort(function(a, b) {
...
async function loadViewAllBatch() {
  ...
  try {
    ...
    if (hasFilters) {
      ...
      const url = `${BASE_URL}/discover/${mediaType}?api_key=${API_KEY}${extraParams}${filterParams}&sort_by=${sortBy}&vote_average.gte=5&page=${viewAllState.page}&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_ALL.join(',')}`;
      const res = await fetch(url);
      data = await res.json();
      let results = applyAllFilters(data.results, mediaType);
      if (key === 'kdrama') {
...
async function init() {
  try {
    ...
    // Fetch data — trending this week in US
    ...
    // Vivamax — hindi isasama kung Family Mode ON
    let vivamaxData = { results: [] };
    if (!familyMode) {
      vivamaxData = await fetchVivamaxMovies(1);
    } else {
      // Itago ang Vivamax row
      const vivamaxRow = document.querySelector('.row:has(#vivamax-list)');
