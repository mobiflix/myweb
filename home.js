...
// ===== EXCLUDED GENRES (Game Show, Reality, Variety, Talk, Soap, Music, Documentary, News, Politics, Kids, Animation) =====
const EXCLUDED_GENRES = [10764, 10767, 10766, 10763, 10768, 10762, 16, 99, 10402];
const EXCLUDED_GENRES_STR = EXCLUDED_GENRES.join(',');

// ===== KOREAN SERIES EXCLUDE =====
const KOREAN_EXCLUDE_GENRES = [10764, 10767, 10766, 10763, 10768, 10762, 16, 99, 10402];
...
const PROVIDER_LOGOS = {
  ...
  'AMC+': 'https://upload.wikimedia.org/wikipedia/commons/2/2e/AMC%2B_logo.svg',
...
function checkDailyUpdate() {
  ...
  if (lastUpdate !== today) {
    ...
    console.log('[MobiFlix] Daily update triggered for', today);
    if ('caches' in window) {
...
function filterExcludedGenres(results) {
  return (results || []).filter(function(item) {
    const genres = item.genre_ids || [];
    if (genres.length === 0) return true;
    return !genres.some(function(g) {
...
async function fetchTrending(type, page) {
  ...
  results = filterExcludedGenres(results);
...
async function fetchMostWatched(mediaType, page) {
  ...
  let url = `${BASE_URL}/discover/${mediaType}?api_key=${API_KEY}` +
    ...
    `&without_genres=${EXCLUDED_GENRES_STR}`;
...
async function fetchOngoingTV(page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&page=${page}&first_air_date.lte=${today}&vote_count.gte=50&with_status=0|1&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_STR}`;
  const res = await fetch(url);
  const data = await res.json();
...
async function fetchCompletedTV(page) {
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&page=${page}&with_status=3|4&vote_count.gte=100&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_STR}`;
  const res = await fetch(url);
  const data = await res.json();
  let results = filterReleased(data.results);
  results = filterExcludedGenres(results);
...
// ============================================================
// ADULT SECTION (Profile)
// ============================================================

async function renderAdultSection() {
  const grid = document.getElementById('adult-grid');
  const empty = document.getElementById('adult-empty');

  if (!grid) return;

  grid.innerHTML = '<div style="color:#666;padding:10px 0;grid-column:1/-1;">Loading...</div>';

  try {
    const data = await fetchVivamaxMovies(1);
    grid.innerHTML = '';
    ...
    grid.innerHTML = '<div style="color:#666;padding:10px 0;grid-column:1/-1;">Failed to load.</div>';
  }
}
...
async function generateNotifications() {
  ...
  try {
    ...
    const newMoviesRes = await fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&sort_by=primary_release_date.desc&primary_release_date.gte=${dateStr}&primary_release_date.lte=${todayStr}&vote_count.gte=20&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_STR}`);
    const newMovies = await newMoviesRes.json();

    const newTVRes = await fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=first_air_date.desc&first_air_date.gte=${dateStr}&first_air_date.lte=${todayStr}&vote_count.gte=20&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_STR}`);
    ...
    (newMovies.results || []).slice(0, 5).forEach(function(item) {
      ...
      notifications.push({
        ...
        poster_path: item.poster_path, type: 'new_release',
        message: 'New movie released!', createdAt: Date.now(), read: false
      });
    });
    ...
    (newTV.results || []).slice(0, 5).forEach(function(item) {
      ...
      notifications.push({
        ...
        poster_path: item.poster_path, type: 'new_release',
        message: 'New TV series released!', createdAt: Date.now(), read: false
      });
    });
...
function renderNotifications() {
  ...
  list.forEach(function(notif) {
    ...
    img.alt = notif.title;
    img.loading = 'lazy';
    img.src = notif.poster_path ? `${IMG_W500}${notif.poster_path}` : 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 75" fill="%23333"><rect width="50" height="75"/></svg>';
...
function renderTop10(items, containerId, mediaType) {
  ...
  const filtered = filterExcludedGenres(released);
...
function clearFilters(pageKey) {
  ...
  if (pageKey === 'view-all') {
    viewAllState.filters = { sort: 'popularity.desc' };
    viewAllState.page = 1;
    viewAllState.hasMore = true;
    viewAllState.seenIds = new Set();
    document.getElementById('view-all-grid').innerHTML = '';
    ...
    providerPageState.filters = { sort: 'popularity.desc' };
    providerPageState.page = 1;
    providerPageState.batchCount = 0;
    providerPageState.hasMore = true;
    providerPageState.seenIds = new Set();
    ...
    genrePageState.filters = { sort: 'popularity.desc' };
    genrePageState.page = 1;
    genrePageState.hasMore = true;
    genrePageState.seenIds = new Set();
    document.getElementById('genre-page-grid').innerHTML = '';
  ...
  searchTimeout = setTimeout(async () => {
    try {
      ...
      const movies = (movieData.results || []).map(function(m) {
        m.media_type = 'movie';
        return m;
      }).filter(function(m) { return filterExcludedGenres([m]).length > 0; });

      const tvs = (tvData.results || []).map(function(t) {
        t.media_type = 'tv';
        return t;
      }).filter(function(t) { return filterExcludedGenres([t]).length > 0; });
...
function openViewAll(key) {
  ...
  viewAllState = { key: key, page: 1, maxPages: 500, loading: false, hasMore: true, initialized: true, seenIds: new Set(), filters: { sort: 'popularity.desc' } };
...
async function loadViewAllBatch() {
  ...
  try {
    ...
    if (hasFilters) {
      ...
      const url = `${BASE_URL}/discover/${mediaType}?api_key=${API_KEY}${extraParams}${filterParams}&sort_by=${sortBy}&page=${viewAllState.page}&without_original_language=${INDIAN_LANGS.join('|')}&without_genres=${EXCLUDED_GENRES_STR}`;
      const res = await fetch(url);
      data = await res.json();
      let results = filterReleased(data.results);
      if (key === 'kdrama') {
      ...
      data.results = filterExcludedGenres(results);
    } else {
      if (key === 'movies') data = await fetchMostWatched('movie', viewAllState.page);
      else if (key === 'tv') data = await fetchMostWatched('tv', viewAllState.page);
