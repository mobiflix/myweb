...
// Stack ng mga bukas na "layer" (page o modal)
let layerStack = [];
...
// Maglagay ng 2 entries: isang "trap" at isang "home" entry.
// Kapag nag-back ang user, mahuhulog siya sa trap at doon mag-confirm.
history.pushState({ mobiflixTrap: false }, '');
history.pushState({ mobiflixHome: true }, '');
...
window.addEventListener('popstate', function(e) {
  ...
  // Nasa homepage na at nag-back — tanungin kung gusto umexit
  ...
  if (wantExit) {
    // Tuloy ang pag-exit — i-back ulit
    exitConfirmActive = false;
    history.back();
  } else {
    // Hindi mag-eexit — i-push ulit ang home entry para may maharang ulit
    history.pushState({ mobiflixHome: true }, '');
    exitConfirmActive = false;
  }
});
...
// ✅ MGA BAGONG FUNCTION PARA SA BOTTOM NAV (Movies at Series)
...
function goHome() {
  ...
  // ✅ I-reset ang exit guard (2 entries ulit)
  history.pushState({ mobiflixTrap: false }, '');
  history.pushState({ mobiflixHome: true }, '');
}
