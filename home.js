...
const LAST_ITEM_KEY = 'mobiflix_last_item';
...
// ============================================================
// PLAY TRAILER — PUMUNTA SA trailer.html
// ============================================================

function playTrailer() {
  if (!currentTrailerKey) return;

  const title = currentItem ? (currentItem.title || currentItem.name || 'Trailer') : 'Trailer';

  const trailerURL = 'trailer.html' +
    '?key=' + encodeURIComponent(currentTrailerKey) +
    '&title=' + encodeURIComponent(title);

  console.log('[MobiFlix Trailer] Opening trailer page:', trailerURL);
...
// ============================================================
// PLAY NOW — PUMUNTA SA player.html
// ============================================================
...
function playNow() {
  ...
  console.log('[MobiFlix Player] Opening player page:', playerURL);
...
function playEpisode(tvId, seasonNumber, episodeNumber) {
  ...
  console.log('[MobiFlix Player] Opening episode page:', playerURL);
...
async function showDetails(item) {
  ...
  try {
    sessionStorage.setItem(LAST_ITEM_KEY, JSON.stringify({
      id: item.id,
...
async function init() {
  try {
    ...
    try {
      const lastItemStr = sessionStorage.getItem(LAST_ITEM_KEY);
      if (lastItemStr) {
        lastItemData = JSON.parse(lastItemStr);
        sessionStorage.removeItem(LAST_ITEM_KEY);
      }
    } catch (e) {}
    ...
    if (lastItemData && lastItemData.id) {
      setTimeout(function() {
        fetchFullDetails(lastItemData.id, lastItemData.media_type);
      }, 800);
    }
