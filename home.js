...
// ===== STREAMING PROVIDERS (dinagdagan) =====
...
const HOME_ROWS = {
  ...
  anime:   { name: 'Anime',      icon: '🎌', media: 'tv',    type: 'anime' }
};
...
window.addEventListener('popstate', function(e) {
  ...
  const openPages = [
    'search-modal', 'more-page', 'my-list-page', 'provider-page',
    'genre-page', 'ongoing-page', 'completed-page', 'vivamax-page',
    'view-all-page', 'user-profile-page', 'person-page'
  ];
...
// ===== MOST WATCHED OF THE YEAR (para sa View All) =====
async function fetchMostWatched(mediaType, page) {
  ...
  if (mediaType === 'movie') {
    url += `&primary_release_date.gte=${currentYear}-01-01&primary_release_date.lte=${currentYear}-12-31`;
  } else {
    url += `&first_air_date.gte=${currentYear}-01-01&first_air_date.lte=${currentYear}-12-31`;
  }
  ...
  // Sort: mataas na popularity + vote_count muna, mababa ang rating sa dulo
  results.sort(function(a, b) {
    const scoreA = (a.popularity || 0) + (a.vote_count || 0) / 10 + (a.vote_average || 0) * 5;
    const scoreB = (b.popularity || 0) + (b.vote_count || 0) / 10 + (b.vote_average || 0) * 5;
...
async function fetchKDramaTopRated(page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
    `&with_origin_country=KR` +
...
async function fetchAnimeTopRated(page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
    `&with_genres=16` +
    `&with_origin_country=JP` +
...
// ===== PERSON CREDITS (para sa cast click) =====
...
// ===== BAGONG NOTIFICATION: New Episodes for Ongoing TV/Kdrama/Anime =====
async function generateNotifications() {
  ...
  try {
    ...
    // New movies
    const newMoviesRes = await fetch(`${BASE_URL}/discover/movie?api_key=${API_KEY}&sort_by=primary_release_date.desc&primary_release_date.gte=${dateStr}&primary_release_date.lte=${today.toISOString().split('T')[0]}&vote_count.gte=20&without_original_language=${INDIAN_LANGS.join('|')}`);
    const newMovies = await newMoviesRes.json();

    // New TV shows
    const newTVRes = await fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=first_air_date.desc&first_air_date.gte=${dateStr}&first_air_date.lte=${today.toISOString().split('T')[0]}&vote_count.gte=20&without_original_language=${INDIAN_LANGS.join('|')}`);
    const newTV = await newTVRes.json();

    // Ongoing TV shows na may bagong episodes (with_status 0 or 1)
    const ongoingRes = await fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&first_air_date.lte=${today.toISOString().split('T')[0]}&vote_count.gte=50&with_status=0|1&without_original_language=${INDIAN_LANGS.join('|')}&page=1`);
    ...
    // Ongoing TV shows - new episode notifications
...
// ============================================================
// PERSON PAGE (para sa cast click)
// ============================================================
...
async function loadViewAllBatch() {
  ...
  try {
    ...
    if (hasFilters) {
      ...
      if (key === 'anime') extraParams = '&with_genres=16&with_origin_country=JP';
      ...
      // Default: Most watched of the year, sorted by popularity
      if (key === 'movies') {
        ...
        data = await fetchAnime(viewAllState.page);
      }
    }
    ...
    // Sort results: most watched/popularity muna, mababa rating sa dulo
...
async function init() {
  try {
    ...
    const [moviesData, tvData, kdramaData, animeData, vivamaxData] = await Promise.all([
